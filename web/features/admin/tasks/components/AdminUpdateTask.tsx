...
import { ChevronDownIcon, Edit, PlusIcon, Trash } from "lucide-react";
...
export default function AdminUpdateTask({ taskId }: { taskId: string; }) {
    const { removeTask, updateTask } = useAdminTasksStore();
    ...
    const onSubmit = async (data: TaskFormData) => {
        try {
            ...
            const task = await adminTasksService.update({ ...finalData, id: taskId });
            updateTask(task);
            reset();
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
            <Card className="max-w-2xl mx-auto">
                <CardHeader>
                    ...
                    <CardAction>
                        {taskId && <Button variant={"outline"} type="button" onClick={handleCreate}><PlusIcon /> Create New Task</Button>}
                    </CardAction>
                </CardHeader>
