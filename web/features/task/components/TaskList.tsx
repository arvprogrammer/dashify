...
export default function TaskList({ page = 1 }: { page: number; }) {
    const { tasks, meta, fetchTasks, loading } = useTasksStore();
    ...
    const toggleCompleted = async (task: Task) => {
        try {
            ...
            const updated = await tasksService.update(task);
            updateTask(updated);
    ...
    const handleUpdate = async (task: Task) => {
        router.push(`/dashboard/tasks/${task.id}/update`);
    };
    ...
    return (
        <div>
            ...
            {tasks.length > 0 && (<>
                {tasks.map((task) => (<Card key={task.id} className="max-w-2xl mx-auto mt-6">
                    <CardHeader>
                        <div className="flex items-center gap-2">
                            <Tooltip>
                                ...
                                <TooltipContent>
                                    <p>Change to {task.status === 'DONE' ? 'To Do' : 'Completed'}</p>
